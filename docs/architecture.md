...
- **Decision:** Codify the following rules for the Medusa backend:
  ...
  7. **`apps/medusa/tsconfig.json` `outDir` must be `./.medusa/server`** (not `./dist`). Medusa's build compiler respects `outDir`, but Medusa Cloud's deployment runtime cd's into `.medusa/server/` to run `medusa db:migrate` and `medusa start`. The CLI's `isLocalMedusaProject()` check reads `./package.json` from that cwd looking for `@medusajs/medusa` or `@medusajs/cli` in deps. If the build wrote to `dist/` instead, `.medusa/server/` is empty and deployment fails with "must be run inside a Medusa project".
