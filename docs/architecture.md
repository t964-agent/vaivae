...
- **Decision:** Codify the following rules for the Medusa backend:
  ...
  10. **`apps/medusa` `build` must remove stale output and assert required artifacts exist:** `rm -rf .medusa/server && medusa build && test -f .medusa/server/package.json && test -f .medusa/server/medusa-config.js`. Medusa v2.14 can print config-load errors but still exit successfully through the Turbo task, so an explicit artifact check prevents Cloud from proceeding to migration with a missing/bad build.
