...
- **Decision:** Codify the following rules for the Medusa backend:
  ...
  8. **The root `package.json` must list `@medusajs/cli` in `dependencies`** (even though the package is only used by `apps/medusa`). Medusa Cloud's automatic migration runner (per the "Database" docs) invokes `medusa db:migrate` after the build; in monorepo deployments it cd's to the repo root, not to the medusa subdir. The CLI's `isLocalMedusaProject()` check reads `./package.json` from cwd and looks for `@medusajs/medusa` OR `@medusajs/cli` in deps/devDeps. Listing `@medusajs/cli` at the root makes the check succeed from any cwd inside the repo. The version must match the monorepo lockstep (currently `2.14.2`).
