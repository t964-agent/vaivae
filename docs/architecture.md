...
### ADR-016: Medusa Cloud Build and Config-Load Constraints

- **Status:** Accepted (corrects assumptions in [§3.5.11](#3511-critical-compatibility-gotchas))
- **Context:** During the first Medusa Cloud deployment (May 2026), `medusa build` and the automatic `db:migrate` step failed with misleading symptoms. Investigation revealed two important Medusa/Cloud behaviors. First, Medusa's `getConfigFile` wraps config loading in a try/catch and returns `{ configModule: null }` when anything throws; with `throwOnError: false` during `medusa build`, this surfaces as the opaque `TypeError: Cannot read properties of null (reading 'admin')`. Second, Medusa CLI project detection for commands such as `db:migrate` reads only the current working directory's `package.json`; if Cloud's deploy step lands outside a valid Medusa app/build root, it emits `The "db/migrate" command must be run inside a Medusa project`. A Cloud-like `turbo prune @vaivae/medusa --docker` reproduction found the hidden upstream issue: the pruned build tree omitted root `tsconfig.base.json`, while `apps/medusa/tsconfig.json` extended `../../tsconfig.base.json`, causing ts-node config loading to fail before a valid `.medusa/server` artifact was emitted.
- **Decision:** Codify the following rules for the Medusa backend:
  1. **Medusa Cloud project root must be `apps/medusa`** (no leading slash in the dashboard field). The storefront root must remain blank because Vercel deploys the storefront.
  2. **`medusa-config.ts` must not import from `./src/*` at the top level.** Medusa's CLI loads the config via plain CJS `require()` before the rest of the app runtime is initialized. Read `process.env` directly; substitute build-time placeholders when `process.argv.includes("build")`.
  3. **`@medusajs/cli` and admin-build packages (`@medusajs/admin-vite-plugin`, `@medusajs/admin-shared`, `@medusajs/admin-sdk`, `@medusajs/dashboard`, `@medusajs/draft-order`) must live in `apps/medusa` `dependencies`, not `devDependencies`.** Medusa Cloud's project validator only inspects `dependencies` (production-install behavior) and reports them missing otherwise.
  4. **`apps/medusa/package.json` must NOT use pnpm `catalog:` references.** Medusa Cloud's validator parses package.json statically and treats `"catalog:"` as the literal version string. Inline real versions in this one file; the storefront keeps catalog refs (Vercel + pnpm handle them).
  ...
  6. **`apps/medusa/tsconfig.json` must be self-contained and must not extend the root `tsconfig.base.json`.** `turbo prune --docker` does not reliably copy root tsconfig files into the pruned build context. Duplicate the required strict compiler flags locally and keep them in sync with the root base config.
  7. **`apps/medusa/tsconfig.json` must include a `ts-node` override block** with `transpileOnly: true`, `module: "commonjs"`, `moduleResolution: "node"`, and `verbatimModuleSyntax: false`. The strict ESM-style compiler settings break ts-node's CJS require hook used by Medusa CLI at config-load time. (The Medusa starter uses `"ts-node": { "swc": true }` for the same purpose; we use plain transpileOnly because we don't ship `@swc/core` in this app.)
  8. **The root `package.json` `prepare` script must be tolerant of missing devDependencies:** use `"prepare": "husky || true"`. Medusa Cloud's build performs a `pnpm install --prod` pass after the build step which strips devDeps; the unguarded `husky` invocation otherwise crashes the build.
  9. **`apps/medusa/tsconfig.json` `outDir` must be `./.medusa/server`** (not `./dist`). Medusa Cloud deploys the contents of `.medusa/server`.
  10. **`apps/medusa` `build` must preflight-load `medusa-config.ts`, remove stale output, and assert required artifacts exist:** `node -r ts-node/register -r tsconfig-paths/register -e "process.argv.push('build'); require('./medusa-config')" && rm -rf .medusa/server && medusa build && test -f .medusa/server/package.json && test -f .medusa/server/medusa-config.js`. Medusa v2.14 can print config-load errors but still exit successfully through the Turbo task, so the preflight and explicit artifact checks prevent Cloud from proceeding to migration with a missing/bad build.
  11. **`medusa-config.ts` must bind the final config to `const __result = defineConfig(...)` before `module.exports = __result`.** Medusa Cloud appends `module.exports = __result` while adapting the config for managed infrastructure; if the binding is absent, Cloud builds fail with `ReferenceError: __result is not defined` before `medusa build` starts.
  12. **`apps/medusa/tsconfig.json` must set `noPropertyAccessFromIndexSignature: false`.** Medusa Cloud injects `process.env.REDIS_URL` while adapting `medusa-config.ts`; the root strict flag rejects that generated code with `TS4111`. Keep this relaxation scoped to the Medusa app only.
  13. **Do not list Medusa runtime packages in the root `package.json`.** The root is a workspace orchestrator, not a Medusa app. Making it look like a Medusa project hides Cloud cwd/configuration mistakes and can move the failure from project detection to config loading.
  14. **Exclude generated Medusa output from workspace package discovery** with `!apps/medusa/.medusa/**` in `pnpm-workspace.yaml`, even though the current `apps/*` pattern does not recurse. This protects future workspace-pattern changes.
- **Alternatives considered:**
  ...
  - _Keep root Medusa dependencies to satisfy project detection:_ rejected after repeated Cloud failures; it did not fix the migration runner and made the repo root masquerade as an app root.
- **Consequences:**
  - Medusa app build is self-contained under `apps/medusa` and compatible with Cloud's pruned Docker context.
  - Build failures surface at the build step instead of later as misleading `db/migrate` project-detection errors.
  - Future contributors must remember that `medusa-config.ts` and `apps/medusa/tsconfig.json` are Cloud-runtime boundaries, not normal workspace files.
