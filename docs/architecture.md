...
- **Decision:** Codify the following rules for the Medusa backend:
  ...
  11. **Do not list Medusa runtime packages in the root `package.json`.** The root is a workspace orchestrator, not a Medusa app. Making it look like a Medusa project hides Cloud cwd/configuration mistakes and can move the failure from project detection to config loading.
  12. **Exclude generated Medusa output from workspace package discovery** with `!apps/medusa/.medusa/**` in `pnpm-workspace.yaml`, even though the current `apps/*` pattern does not recurse. This protects future workspace-pattern changes.
